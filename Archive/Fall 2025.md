...
# Late December
Today 12/23:
- [ ] Better understanding of latent ODEs
	- [ ] What are all the pieces of Patrick Kidger's example?
	- [ ] Given the latent ODE paper he cited a first pass
	- [ ] GRU vs MLP 
- [ ] Working understand of adjoint method
- [ ] Plan journal visualizations for 2BP
- [ ] Figure out how 2BP is better
	- [ ] I want new metric visualizations
	- [ ] And new metrics
		- [ ] How can we track how well the model is tracking the physics of the problem?

12/29, working on above tasks
For tomorrow, I want:
- [ ] Plan of narrative for journal paper
- [ ] verify acceleration metrics 
	- [ ] simplify acceleration metrics
- [ ] figure out when mlp_4D not signed is better
	- [ ] probably need new metrics
	- [ ] try training for 3D
- [ ] Visualizations
	- [ ] 3 good ones would be ideal
		- [ ] at least one should show an actual physical orbit
- [ ] Latent ODE work
	- [ ] Review how they work
## Continuing 2BP work
We thought that changing our output layer from mlp_4D_signed to something different would help. So far it has not in the case of planar 2BP.
Thoughts on this:
- [ ] Perhaps we have better results with 3D than we would have otherwise - the planar constraint is applied outside of the output layer 

Currently:
Review how the acceleration metrics are being calculated. It seems overcomplicated.
Brainstorm and implement new metrics which test if physics are being obeyed.
Try training on 3D data with mlp_4D_signed vs one of the other ones
...
The class `AccelerationMetric` has an `__init__` (initializer/initialization method) which stores the metric name and `true_dynamics` function. The dynamics are then accessible via`self.true_dynamics` when we call it. We set the `true_dynamics` when we construct a new `AccelerationMetric` object. If `true_dynamics` was not passed, calling `AccelerationMetric()` would throw a `TypeError` (missing required positional argument) since we don't have a default value.

Lamda functions: `lambda` denotes an anonymous function, which is a small inline function you can define without a name. It has the form lambda args: expression and returns the value of that expression. Here it’s used to define a per-trajectory function to pass into jax.vmap. Ex.: 
```
square = lambda x: x * x
print(square(4))  # 16
```
which is in contrast to our typical definition of a function using `def`. For example, the above code would more classically be written as:
```
def square(x):
    return x * x
```

`jax.vmap` vectorizes a function: it automatically maps it over array axes so you can write code for a single example and run it over batches (or time steps) without explicit Python loops. It’s like map, but returns a stacked array and works with JAX’s JIT/grad. Here it’s used twice (nested) to avoid explicit loops (recall that our dimensions are N, T, D=6):

- Inner vmap(self.true_dynamics, in_axes=(0, 0)) applies true_dynamics over the time axis of a single trajectory: it takes t_seq and y_seq (both shaped [T, ...]) and returns [T, ...].
- Outer vmap(lambda t_seq, y_seq: ...) applies that per‑trajectory function over the batch axis, so it runs for each (t_seq, y_seq) pair in ti and yi.

So the overall effect is: for a batch of trajectories, compute true_dynamics(t, y) at every time step for every batch element, without writing any Python loops.
self.true_dynamics is callable:
```
class TBPDynamics:
    def __init__(self, mu):
        self.mu = mu

    def __call__(self, t: Float, state: Float[Array, "6"]) -> Float[Array, "6"]:
...
`model` is an instance of the `NeuralODE` class (an instance of a class is just an object). `model.func` is an instance of `Func` which wraps the MLP plus feature/output layers and constraints. The `__call__` method of Func allows us to call Func which looks like: input a Cartesian position and velocity -> transform via feature layer -> pass into mlp -> apply the output layer 
`mlp_outputs[0:3]` are acceleration, and `mlp_outputs[3:6]` are velocity (but we just use `state[3:6]).
It doesn’t assume an ordering by itself. diffrax just expects self.func(t, y, ...) to return dy/dt with the same shape as y0. So the ordering is whatever your state uses. In your setup, if y = [pos, vel], then dy/dt should be [vel, accel] (position derivatives first, then velocity derivatives). If self.func returned a different ordering, the integration would be wrong.

Note to self: in the conference paper, we were actually calculating error based on acceleration and velocity because when we apply jax.vamp, we get the full state derivative (acceleration and velocity) at each timestep for each initial condition because both true_dynamics reutnrs 6 outputs (vel and acc), and model.func returns 6 outputs. 
# December 12

mlp_4D_unit_softplus: oglbia0l
mlp_4D_logmag_unit_exp: fk5tthtg
mlp_4D_signed: sxrzqpz2
mlp_4D_unit: 9hp5iemb
TODO:
- [ ] fix that one visualization
- [ ] input metric to record when acceleration is not collinear with position
- [ ] Learn about Latent ODEs
- [ ] Try training with different seeds
- [ ] Put together presentation

## Fixing visualization:
- [x] From the test-sweep-12-10 wandb group, take out the spherical harmonics feature layer
	- done - the group is now test-sweep-12-10v2
- [ ] sanity cheak - in the visualization, train_1 test_1 with input feature mlp_4D_signed and segment_length=4says it has a mean acceleration error of 514.66% which cannot be correct. let's find this run_id and run it through analyzse_2BP_debug.py. Let's also find the same for train_100 to see if the order has gotten switched or something
	- train_1 run_id: l7mk3b4v
	- train_10 run_id: 3wa4ume2
	- train_100 run_id: arsbqf1i
...
| :------- | :-------------- | :------------ | -------------------------------: | ------------------------------------: | ----------------------------------: | -----------------------------------: | --------------------------------------: | --------------------------------------: | ---------------------------------: | --------------------------------: | -------------------------------------: | -----------------------------------: | ------------------------------------: | ---------------------------------------: | ---------------------------------------: | ----------------------------------: | ---------------------------------: | --------------------------------------: | ------------------------------------: | -------------------------------------: | ----------------------------------------: | ----------------------------------------: | -----------------------------------: |
| l7mk3b4v | l7mk3b4v        | mlp_4D_signed |                              515 |                                  3.13 |                                   0 |                               0.0973 |                                    1.06 |                                  -0.213 |                             -0.183 |                               430 |                                   3.82 |                                    0 |                                  2.26 |                                    0.847 |                                    0.945 |                               0.277 |                                358 |                                    4.36 |                                     0 |                                   1.96 |                                     0.869 |                                      1.05 |                                0.318 |
| 3wa4ume2 | 3wa4ume2        | mlp_4D_signed |                             4.15 |                                  1.23 |                                   0 |                                 1.45 |                                   0.433 |                                   0.664 |                              0.283 |                              6.08 |                                    1.8 |                                    0 |                                  3.59 |                                    0.351 |                                    0.902 |                               0.231 |                               5.03 |                                    1.46 |                                     0 |                                   3.09 |                                     0.556 |                                     0.903 |                                0.264 |
| arsbqf1i | arsbqf1i        | mlp_4D_signed |                             3.02 |                                 0.576 |                                   0 |                                0.739 |                                   0.541 |                                   0.838 |                              0.268 |                              3.02 |                                  0.602 |                                    0 |                                  2.97 |                                    0.529 |                                    0.861 |                               0.232 |                               2.48 |                                   0.596 |                                     0 |                                    3.4 |                                     0.409 |                                     0.743 |                                0.193 |
# December 11

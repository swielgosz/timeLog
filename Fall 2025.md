...
```
we get up to 16 workers in parallel. But if we also have OMP_NUM_THREADS and MKL_NUM_THREADS set to 1, each worker is single-threaded so total effective CPU concurrency ~= 16 threads when we allowed 64. This means we are underutilizing compute and have slower runs
To do:

...
Big error that I discovered: we had a single ray sweep running. None of the temporary files that were in tmp/ray/ were deleted, so we ended up with ~800 GB of temporary files. When we run out of disk space, the sweep crashes and Docker crashes and everything crashes. To get around this, we can batch how many 

OOM error is about memory (RAM/GPU), not CPU

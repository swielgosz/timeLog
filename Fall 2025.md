...
Difference between ray.int(num_cpus=16) and ray.remote(num_cpus=4) is that num_cpus in ray.init sets the cluster-wide CPU capacity Ray should schedule against (how many logical CPU slots are available) and num_cpus in ray.remote sets how many of those slots a particular task reserves whil it runs. ray.init declares the pool size, num_cpus declares each job's requirement

artifacts are being saved to /tmp/ray/ which took up all the memory. We need to prevent this

``` python
"""
Train a 2BP mapper-based Neural ODE with Jacobian linearity regularization.

Functionality:
- Encoder: y0 -> latent posterior (mean/logstd), then reparameterized z0
...
Example usage:
    python scripts/training/train_latent_2BP_mapper_jacobian_reg.py
"""
...
# Keep CPU-only and limit threads to match GRU Jacobian-reg training runtime setup.
...
from neuralODE.latentODE import JacobianRegularizedMapperLatentODE
...
def loss_fn(
    model: JacobianRegularizedMapperLatentODE,
    t_batch,
    y_batch,
    key,
    *,
...
def build_model(
    ...
    *,
    ...
    """
    Build mapper-based Neural ODE with Jacobian regularization target matrix.
    """
    return JacobianRegularizedMapperLatentODE(
        data_size=data_size,
...
def save_jacobian_mapper_checkpoint(
    config: dict,
    model: JacobianRegularizedMapperLatentODE,
    path: Path,
) -> None:
    """
    Save Jacobian-regularized mapper model checkpoint with JSON header.
    """
...
def load_jacobian_mapper_checkpoint(
    path: Path,
) -> tuple[dict, JacobianRegularizedMapperLatentODE]:
    """
    Load Jacobian-regularized mapper model checkpoint.
    """
    with path.open("rb") as f:
        ...
        model = JacobianRegularizedMapperLatentODE(
            ...
            width_size=int(config["width_size"]),
...
def main():
    """
    Train and log Jacobian-regularized mapper Neural ODE on normalized 2BP data.
    """
    config = {
        ...
        "num_trajs": 10,
        "seed": 0,
        "encoder_type": "explicit_mapper",
        "linearity_regularization": "jacobian_target_matrix",
    }
    wandb_run = wandb.init(
        project="neuralODEs",
        entity="mlds-lab",
        group="latent-ode-mapper-jacobian-reg-2bp",
        config=config,
    )
    ...
    segment_cache = {}
    best_loss = float("inf")
    best_path = Path("files/models") / f"best_latent_2bp_mapper_jac_{wandb_run.id}.eqx"
    ...
    progress_bar = tqdm(
        total=total_steps,
        desc="Latent 2BP mapper Jacobian-reg training",
        unit="step",
    )
    global_step = 0
        ...
        for step in range(int(steps)):
            ...
            if step % 20 == 0 or step == int(steps) - 1:
                ...
                if config["save_best"] and float(total) < best_loss:
                    ...
                    save_jacobian_mapper_checkpoint(config, model, best_path)
                print(
    ...
    output_dir = Path("files/figures/latent_2bp_mapper_jacobian_reg")
    ...
    for orbit_idx in [0, 1]:
        ...
        ax.plot(y_hat_phys[:, 0], y_hat_phys[:, 1], "--", label="jac-reg recon XY")
        ax.set_aspect("equal", adjustable="box")
        ax.set_xlabel("x [m]")
        ax.set_ylabel("y [m]")
        ax.set_title(f"Jacobian-reg latent ODE reconstruction (XY, orbit {orbit_idx})")
        ax.legend()
        fig.tight_layout()
        local_plot_path = (
            output_dir
            / f"latent_jacobian_reconstruction_{wandb_run.id}_{orbit_idx}.png"
        )
    ...
    if config.get("save_best", False) and best_path.exists():
        try:
            _, model_to_save = load_jacobian_mapper_checkpoint(best_path)

...
dt = (t1 - t0) / 10
```
If performance degrades, adaptive solver was doing work and low step count wasn't hiding issues. If performance is similar, learned dynamics are simple and bottleneck is model expressivity/inductive bias, not solver. This suggests > Your performance ceiling is coming from **model structure or learning objective**, not stiffness or solver pathologies. But also very small avg_dt relative to the time scale of your dynamics indicates stiffness.

You have a few pragmatic options to assess stiffness. A Jacobian spectrum check is one, but it’s not the only (or always best) indicator.

Practical stiffness signals (low overhead):

Rejected step rate: high num_rejected_steps / num_accepted_steps is a strong indicator. You already log solver_reject_rate_mean.
Average step size: very small avg_dt relative to the time scale of your dynamics indicates stiffness.
Explicit vs implicit solver speed: run the same batch with Tsit5 and a stiff solver (e.g., BDF or Kvaerno5 in Diffrax). If the implicit solver is faster or more stable, that’s a practical “stiffness detected” signal.
Jacobian spectrum (more principled, more expensive):

Compute eigenvalues of the Jacobian ∂f/∂y at representative points.
A system is often considered stiff if the ratio |Re(λ_max)| / |Re(λ_min)| is large (or if there are eigenvalues with large negative real parts).
This is expensive in high dimensions, but you can:
sample a subset of time points,
compute Jacobian via jax.jacrev,
run jax.numpy.linalg.eigvals or power iteration for dominant eigenvalues,
or compute just a few largest magnitude eigenvalues with Arnoldi if you have a sparse structure.
What I recommend for your setup:

Add a lightweight “stiffness score” based on:
solver_reject_rate_mean
solver_avg_dt_mean (normalized by the time span)
optional: compare wall time or steps for Tsit5 vs BDF on a small sample batch.
If you want the Jacobian spectrum, I can implement a function like:

estimate_stiffness_jacobian(model, ts, y0, sample_points=3, max_dim=64)
samples a few y(t) from a trajectory
computes Jacobian of model.func(t, y)
computes spectral radius and stiffness ratio.
Tell me which direction you want:

Step/reject‑rate heuristic (cheap, easy)
Compare explicit vs implicit solver (robust)
Jacobian spectrum (most diagnostic, expensive)
